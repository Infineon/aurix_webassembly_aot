...
fn generate_tests_from_json(mvp_tests_dir:&Path,test_folder:&Path) -> Vec<(String,PathBuf)> {
        ...
        let wasm_modules = commands.iter().filter(|cmd| cmd["type"].as_str().unwrap() == "module")
         ...
         commands.split(|cmd| cmd["type"].as_str().unwrap() == "module").skip(1).enumerate().for_each( |(wasm_module_index, test_suite)| {
            ...
            let test_code = test_suite.iter().enumerate().map(|(test_index,cmd)| {
                    ...
                    let (return_type,expected) = cmd["expected"].as_array().unwrap().iter().next().map(|arg|{
                        let arg = arg.as_object().unwrap();
    ...
    "#,
                        ...
                        assertion = match expected {
                            ...
                            ReturnValue::ArithmeticNanF32 => "assert!(f32::from_bits(result.unwrap().as_u32()).is_nan());".to_string(),
                            ReturnValue::ArithmeticNanF64 => "assert!(f64::from_bits(result.unwrap().as_u64()).is_nan());".to_string(),
                            ReturnValue::CanonicalNanF32 => "assert!(result.unwrap().as_u32()==f32::NAN.to_bits() || result.unwrap().as_u32()==(-f32::NAN).to_bits());".to_string(),
                            ReturnValue::CanonicalNanF64 => "assert!(result.unwrap().as_u64()==f64::NAN.to_bits() || result.unwrap().as_u64()==(-f64::NAN).to_bits());".to_string()
                        }
                    )}).collect::<Vec<_>>().join("\n");

...
pub fn debug() {
    unsafe {
        core::arch::tricore::intrinsics::__debug();
    }
}
...
static mut EXCEPTION_PANIC: fn(u32) -> ! = |x| exit(x);
...
/// This function allow to change at runtime the handler for all trap execept misaligned
/// # Safety
///
/// Don't call it concurrently
pub unsafe fn set_exception_handler(function: fn(u32) -> !) {
    unsafe {
        EXCEPTION_PANIC = function;
    }
}

// since tsim will stop reaching _exit we wrap it by exit and prepare the exit
// status in A14 (interpreted by TSIM). On real hardware has no effect.
